...
export async function GET(
  ...
  try {
    ...
    const payment = await prismadb.incomingPayment.findUnique({
      where: { 
        id: paymentId,
        businessUnitId 
      },
    ...
    console.log('[INCOMING_PAYMENT_GET]', error);
...
export async function PATCH(
  ...
  try {
    ...
    const payment = await prismadb.incomingPayment.update({
      where: { 
        id: paymentId,
        businessUnitId 
      },
    ...
    console.log('[INCOMING_PAYMENT_PATCH]', error);
    return new NextResponse("Internal error", { status: 500 });
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    console.log('[INCOMING_PAYMENT_DELETE]', error);
    return new NextResponse("Internal error", { status: 500 });
  }
}