...
import { createAPInvoice, getAPInvoices } from '@/lib/actions/financials-actions';
import { APInvoiceFilters } from '@/types/financials-types';
...
export async function POST(
  ...
  try {
    ...
    const { bpCode, basePurchaseOrderId, postingDate, dueDate, documentDate, remarks, items } = body;
    ...
    console.log('[AP_INVOICE_POST]', error);
...
export async function GET(
  ...
  try {
    ...
    const filters: APInvoiceFilters = {
      businessUnitId,
    ...
    const result = await getAPInvoices(filters, pagination);
