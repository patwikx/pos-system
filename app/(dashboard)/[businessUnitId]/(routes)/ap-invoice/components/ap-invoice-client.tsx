...
import { Plus, Filter, Download, FileText, DollarSign, AlertTriangle } from "lucide-react";
...
import { columns, APInvoiceColumn } from "./ap-invoice-columns";
import { formatter } from "@/lib/utils";

interface APInvoiceClientProps {
  data: APInvoiceColumn[];
}

export const APInvoiceClient: React.FC<APInvoiceClientProps> = ({ data }) => {
  ...
  const filteredData = useMemo(() => {
    return data.filter((invoice) => {
      const searchContent = `${invoice.docNum} ${invoice.vendorName}`.toLowerCase();
  ...
  return (
    <>
      <div className="flex items-center justify-between">
        <Heading 
          title={`Accounts Payable (${filteredData.length})`} 
          description="Manage vendor bills and track outstanding payables" 
        />
        <div className="flex items-center gap-2">
          <Button onClick={() => router.push(`/${params.businessUnitId}/ap-invoice/reports`)} variant="outline">
            <FileText className="mr-2 h-4 w-4" />
            Reports
          </Button>
          <Button onClick={() => router.push(`/${params.businessUnitId}/ap-invoice/new`)}>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-red-600">{formatter.format(summaryStats.totalBalance)}</div>
          </CardContent>
        </Card>
